...
import { applyExtensionPlan, listExtensions, planExtensionSync, type ExtensionInfo } from "../sync/extensions.js";
...
export function runExport(platformId: string, opts: ExportOptions): number {
  ...
  for (const f of PROFILE_FILES) {
    ...
    if (existsSync(src)) { copyAny(src, join(target, f)); log.success(`saved ${f}`); }
  }
  const snippetsSrc = join(p.paths.userDataDir, "snippets");
  if (existsSync(snippetsSrc)) { copyAny(snippetsSrc, join(target, "snippets")); log.success("saved snippets/"); }
...
export function runImport(bundlePath: string, platformId: string, opts: ImportOptions): number {
  ...
  if (!target) { log.error(`Unknown platform: ${platformId}`); return 1; }

  const bundle = resolve(bundlePath);
  if (!existsSync(bundle)) { log.error(`Bundle not found: ${bundle}`); return 1; }

  const metaPath = join(bundle, "vibe-sync.json");
  if (!existsSync(metaPath)) { log.error(`Missing vibe-sync.json in bundle: ${bundle}`); return 1; }

  const meta = JSON.parse(readFileSync(metaPath, "utf8")) as ProfileMeta;
  log.header(`Importing ${meta.source.displayName} bundle → ${target.def.displayName}`);

  ensureDir(target.paths.userDataDir);
  for (const f of PROFILE_FILES) {
    const src = join(bundle, f);
    if (existsSync(src)) {
      if (opts.dryRun) log.raw(`  ${c.yellow("DRY")} would write ${f}`);
      else { copyAny(src, join(target.paths.userDataDir, f)); log.success(`wrote ${f}`); }
    }
  }
  const snippetsSrc = join(bundle, "snippets");
  if (existsSync(snippetsSrc)) {
    if (opts.dryRun) log.raw(`  ${c.yellow("DRY")} would write snippets/`);
    else { copyAny(snippetsSrc, join(target.paths.userDataDir, "snippets")); log.success("wrote snippets/"); }
  }
