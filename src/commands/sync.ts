...
function parseKinds(opts: SyncOptions): SyncKind[] {
  ...
  if (opts.include?.length) {
    kinds = opts.include.filter((k): k is SyncKind => (SYNC_KINDS as string[]).includes(k));
  }
...
async function pickTargets(source: DetectedPlatform): Promise<DetectedPlatform[]> {
  const installed = detectInstalled().filter((p) => p.def.id !== source.def.id);
  ...
  if (!ids) return [];
  return (ids as string[]).map((id) => resolvePlatform(id)).filter((p): p is DetectedPlatform => Boolean(p));
}
...
export async function runSync(opts: SyncOptions): Promise<number> {
  ...
  if (opts.from) {
    ...
    if (!source) { log.error(`Unknown source platform: ${opts.from}`); return 1; }
    if (!source.installed) {
      log.error(`"${source.def.displayName}" is not installed (${source.paths.userDataDir} not found).`);
      return 1;
    }
  } else {
    source = await pickSource();
    if (!source) { log.error("No installed platforms detected."); return 1; }
  }

  let targets: DetectedPlatform[] = [];
  if (opts.all) {
    targets = detectInstalled().filter((p) => p.def.id !== source!.def.id);
  } else if (opts.to?.length) {
    for (const t of opts.to) {
      const r = resolvePlatform(t);
      if (!r) { log.error(`Unknown target platform: ${t}`); return 1; }
      if (r.def.id !== source.def.id) targets.push(r);
    }
  } else {
    targets = await pickTargets(source);
  }

  if (targets.length === 0) { log.warn("No targets selected."); return 0; }

  log.header(`Sync plan: ${source.def.displayName} → ${targets.map((t) => t.def.displayName).join(", ")}`);
  ...
  log.info(`Prune:       ${opts.prune ? "yes" : "no"}`);
  log.info(`Dry run:     ${opts.dryRun ? c.yellow("yes") : "no"}`);
  log.info(`Backups:     ${opts.noBackup ? c.yellow("disabled") : (opts.backupDir ?? DEFAULT_BACKUP)}`);
  ...
  for (const target of targets) {
    ...
    if (doFiles) {
      const results = syncFiles(source, target, {
        kinds,
      ...
      if (results.length === 0) {
        ...
        for (const r of results) {
          const verb =
            r.action === "wrote" ? c.green("wrote   ") :
            r.action === "merged" ? c.green("merged  ") :
            r.action === "would-write" ? c.yellow("DRY     ") :
            r.action === "would-merge" ? c.yellow("DRY-MRG ") :
            c.dim("skipped ");
          const detail = r.kind === "snippets" ? `${r.files ?? 0} files` : `${r.bytes ?? 0} bytes`;
